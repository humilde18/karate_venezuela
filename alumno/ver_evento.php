...
$query_evento = "SELECT e.nombre AS evento_nombre, e.descripcion, e.fecha, e.lugar 
                 FROM eventos e 
...
$query_dojo_alumno = "SELECT d.nombre AS dojo_nombre, s.nombre AS sensei_nombre, s.apellido AS sensei_apellido
                      FROM usuarios u
                      INNER JOIN dojos d ON u.dojo_id = d.id
                      INNER JOIN usuarios s ON d.sensei_id = s.id
                      WHERE u.id = $id_alumno";
$res_dojo_alumno = mysqli_query($conexion, $query_dojo_alumno);
$dojo_alumno = mysqli_fetch_assoc($res_dojo_alumno);

// Obtener los participantes del evento agrupados por dojo
$query_participantes = "
...
$agrupado = [];
while ($row = mysqli_fetch_assoc($res_participantes)) {
...
<div class="container mt-5">
    ...
    <div class="mb-4">
        <h2><?= htmlspecialchars($evento['evento_nombre']) ?></h2>
    ...
    <?php if ($dojo_alumno): ?>
        <div class="dojo-card">
            <h3>🏯 Mi Dojo: <?= htmlspecialchars($dojo_alumno['dojo_nombre']) ?></h3>
            <p><strong>Sensei:</strong> <?= htmlspecialchars($dojo_alumno['sensei_nombre'] . ' ' . $dojo_alumno['sensei_apellido']) ?></p>
        </div>
    <?php endif; ?>
    ...
    <a href="mis_eventos.php" class="btn btn-primary mt-4">Regresar a Mis Eventos</a>
</div>
